	...
	<div class="row">
		<div class="col-md-8 col-sm-8 col-xs-12 mb-5 main-panel p-5 border rounded">
			...
			<form action="{{ route('gateway.complete', ['gateway' => config('yookassa.alias')]) }}" method="post" id="payment-form">
				...
				<button id="rzp-button" type="submit" class="pay-button sub-btn btn vbtn-outline-success text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3">{{ __('Pay with :x', ['x' => $name]) }}</button>
