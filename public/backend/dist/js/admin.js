$(document).ready(function(){
    ...
    function ajax_request_formdata(post_url, post_data){
        ...
        $.ajax({
            ...
            error: function(jqXHR, textStatus, errorThrown){
                show_error_message('Network problem.'+jqXHR+' '+textStatus+' '+errorThrown);
                result = [];
            }
        });
        return result;
    ...
    function show_success_message(message){
        if ( $('.flash-container').attr("style") ) {
            $('.flash-container').removeAttr("style");
        }
        ...
        $('#success_message_div').delay(5000).fadeOut('slow', function() {
    ...
    function show_error_message(message){
        ...
        $('#error_message_div').delay(5000).fadeOut('slow', function(){
            $(this).addClass('d-none');
            $(this).removeAttr("style");
        });
    ...
    $(document).on('submit', '#add_page', function() {
        $('button').attr('disabled', 'disabled');
    });

    $(document).on('submit', '#edit_page', function() {
        $('button').attr('disabled', 'disabled');
    });
